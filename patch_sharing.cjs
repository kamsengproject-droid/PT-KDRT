const fs = require('fs');
let code = fs.readFileSync('src/pages/DashboardSharingPage.tsx', 'utf8');

// Add imports
code = code.replace(
  /import \{[\s\S]*?\} from 'lucide-react';/,
  `$&
import { TrendingUp as TrendingUpIcon, DollarSign as DollarSignIcon, X } from 'lucide-react';`
);
...
// Add subscribe
code = code.replace(/const unsubExp = subscribeExpenses\('SHARING', setExpenses\);/,
  `$&
...
code = code.replace(/<div className="rounded-xl border border-emerald-200 bg-emerald-50\/40 p-4 shadow-xs">/, 
  `<div 
...
code = code.replace(/<div className="rounded-xl border border-rose-200 bg-rose-50\/40 p-4 shadow-xs">/, 
  `<div 
            ...
            <div className="flex items-center justify-between border-b border-zinc-100 p-6 shrink-0">
              <div>
                <h3 className="text-xl font-black text-rose-900 flex items-center gap-2">
                  <DollarSignIcon className="h-6 w-6 text-rose-600" />
...
fs.writeFileSync('src/pages/DashboardSharingPage.tsx', code);
