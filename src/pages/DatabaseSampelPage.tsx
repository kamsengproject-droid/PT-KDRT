...
export const DatabaseSampelPage: React.FC<DatabaseSampelPageProps> = ({
  ...
  // Database sampel hanya memiliki dua kategori operasional: BARU dan LAMA.
  const { newSamples, oldSamples } = useMemo(() => ({
    newSamples: filteredSamples.filter((sample) => sample.status !== 'SELESAI'),
    oldSamples: filteredSamples.filter((sample) => sample.status === 'SELESAI'),
  }), [filteredSamples]);
  ...
  // Pindahkan sampel antar dua kategori menggunakan status existing.
  const handleMoveSampleToOld = async (sample: AffiliateSample) => {
    try {
      const uid = userProfile?.uid || 'system';
      const name = userProfile?.name || 'User';
      await updateSampleStatus(sample.id!, sample, 'SELESAI', uid, name);
      setSuccessToast(`"${sample.productName}" dipindahkan ke Sampel Lama.`);
    } catch (err: any) {
      setActionError(err.message || 'Gagal memindahkan sampel.');
    }
  };

  const handleMoveSampleToNew = async (sample: AffiliateSample) => {
    try {
      const uid = userProfile?.uid || 'system';
      const name = userProfile?.name || 'User';
      await updateSampleStatus(sample.id!, sample, 'DITERIMA', uid, name);
      setSuccessToast(`"${sample.productName}" dikembalikan ke Sampel Baru.`);
    } catch (err: any) {
      setActionError(err.message || 'Gagal mengembalikan sampel.');
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-zinc-900 via-slate-900 to-zinc-900 rounded-3xl p-6 text-white shadow-xl border border-zinc-800">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mt-1 max-w-xl">
            Pusat terpadu katalog Master Produk affiliate, sampel fisik, progres VT, dan riwayat sampel lama.
          </p>
        </div>
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-zinc-100 text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            <button
                              onClick={() => handleMoveSampleToOld(sample)}
                              className="rounded-lg bg-zinc-100 border border-zinc-200 px-2.5 py-1 text-[11px] font-bold text-zinc-700 hover:bg-zinc-200 transition-colors"
                            >
                              Jadikan Sampel Lama
                            </button>
                        </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-zinc-200 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {!isInvestor && (
                          <>
                            <button
                              onClick={() => handleMoveSampleToNew(sample)}
          ...
          <div className="flex items-center justify-between bg-indigo-50/70 border border-indigo-200 rounded-2xl px-4 py-3">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                Master barang afiliasi. Klik <strong>[ + BELI SAMPEL ]</strong> untuk mencatat unit fisik yang sudah ada di kantor.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredProducts.map((prod) => (
                ...
                >
                  ...
                  {/* Footer & Action: [ + BELI SAMPEL ] — hidden if a physical sample already exists */}
                  <div className="mt-4 pt-3 border-t border-zinc-100 flex items-center justify-between gap-2">
                    {(() => {
                      // "Sampel fisik aktif/tersedia" = status DITERIMA/DIGUNAKAN/SELESAI
                      // (physically arrived at office). DIPESAN/DIKIRIM don't count —
                      // those haven't physically arrived yet, so buying is still valid.
                      const physicalSamples = samples.filter(
                        (s) => s.productId === prod.id && ['DITERIMA', 'DIGUNAKAN', 'SELESAI'].includes(s.status)
                      );
                      if (physicalSamples.length > 0) {
                        return (
                          <button
                            ...
                            className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 py-2 px-3 text-xs font-black cursor-pointer hover:bg-emerald-100 transition-colors"
                            title="Klik untuk lihat detail sampel fisik"
                          >
                            <Check className="h-3.5 w-3.5" />
                            <span>Sampel Fisik Tersedia ({physicalSamples.length}x)</span>
                          </button>
                        );
                      }
                      return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 text-zinc-900 shadow-2xl border border-zinc-200 my-8">
            ...
            <form onSubmit={handleSubmitSample} className="space-y-3.5 text-xs">
              ...
              {!editingSample && (
                <div className="rounded-xl bg-emerald-50 p-3 border border-emerald-200 space-y-2">
                  <div className="flex items-center gap-2 font-bold text-emerald-700">
                    <CheckCircle2 className="h-4 w-4" />
                    <span>Uang Keluar sampel akan dicatat otomatis saat disimpan.</span>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 text-zinc-900 shadow-2xl border border-zinc-200 space-y-4">
            ...
            <div className="pt-2 flex gap-2">
              {(() => {
                const physicalSamples = samples.filter(
                  (s) => s.productId === detailProduct.id && ['DITERIMA', 'DIGUNAKAN', 'SELESAI'].includes(s.status)
                );
                if (physicalSamples.length > 0) {
                  return (
                    <button
                      onClick={() => {
                        ...
                        setActiveTab('SAMPEL');
                        setSearchQuery(detailProduct.productName);
                      }}
                      className="flex-1 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 py-2.5 font-black text-xs hover:bg-emerald-100"
                    >
                      ✓ Sampel Fisik Tersedia ({physicalSamples.length}x) — Lihat Detail
                    </button>
                  );
                }
                return (
