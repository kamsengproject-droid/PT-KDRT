      ...
      autoCreateExpense && totalCost > 0
        ? (async () => {
            ...
            try {
              ...
              // Sampel sudah tersimpan, tetapi pencatatan Uang Keluar adalah bagian
              // wajib dari transaksi sampel. Jangan menelan error karena itu membuat
              // Database Sampel terlihat sukses sementara ledger keuangan tetap Rp0.
              console.error('Auto expense recording failed:', expErr);
              throw expErr;
            } finally {
...
export async function recordSampleExpense(
  ...
  try {
    ...
    // Skipped when the sample was JUST created in this same request (skipDuplicateCheck=true):
    // a brand-new sampleId (freshly generated by addDoc) cannot possibly have a prior expense
    // referencing it, so this read is provably redundant on that path only. Every other caller
    // (e.g. edit flow, manual retry) still runs this check — anti-duplicate is not weakened.
